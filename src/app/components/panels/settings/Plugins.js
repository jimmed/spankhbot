...
function PluginList ({ plugins = TEST_PLUGINS, actions }) {
  return (
    <table className='table'>
      <thead>
        <tr><th>Plugin</th><th>Actions</th></tr>
      </thead>
      <tbody>
...
class PluginListItem extends React.Component {
  ...
  render () {
    ...
    return (
      <tr>
        <td>{plugin.get('displayName')}</td>
        <td>
          <button type='button' onClick={this.toggleActive.bind(this)} className={`button ${plugin.get('enabled') ? 'success active' : 'secondary'}`}>
            {plugin.get('enabled') ? 'On' : 'Off'}
          </button>
          <button type='button' onClick={this.openSettings.bind(this)} className={`button primary hollow`}>
            Settings
          </button>
        </td>
      </tr>
