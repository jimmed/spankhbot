...
class ChatPanel extends React.Component {
  ...
  sendMessage(message) {
    const username = this.props.accounts.getIn([
      this.state.account,
      'profile',
      'name'
    ]);
      ...
      <div className={cx('Panel')}>
        <div className="top-bar">
          <div className="top-bar-left">
            <ul className="menu">
              <li className="menu-text">Chat</li>
            </ul>
          </div>
          ...
          : <div className="ChatDisconnected">
              {accounts.hasIn([account, 'oAuth', 'accessToken']) && channel
                ? connecting
                  ? <button
                      type="button"
                      className="large hollow primary button"
                    >
                      Connecting to{' '}
                      <strong>#{channel.get('display_name')}</strong>&hellip;
                    </button>
                  : <button
                      type="button"
...
class ChatAccountSwitcher extends React.Component {
  ...
  render() {
    var accountsMenu = this.props.accounts
      ...
      .map((account, type) =>
        <li
          key={type}
          style={{ color: '#0a0a0a' }}
          onClick={() => this.onSwitchAccount(type)}
        >
    ...
    return (
      <a
        ref="outer"
        className="input-group-button dropdown button small arrow-only"
        onClick={this.onToggleMenu.bind(this)}
      >
        <span className="show-for-sr">Switch accounts</span>
        {this.state.open &&
          <ul
            className="vertical dropdown menu submenu is-dropdown-submenu js-dropdown-active"
            style={{
              maxWidth: 300,
              right: 8,
              left: 'auto',
    ...
    return messages
      ...
      .reduce((groups, message) => {
        ...
        const isRecentEnough =
          prevTimestamp && message.get('date') - prevTimestamp < 30000;
  ...
  render() {
    ...
    return (
      <div className="ChatItems-list" ref="items">
        ...
        {this.groupMessagesByTime(
          messages
        ).map(({ messages, sender, date }, id) =>
          <div className={`${cx('Message')} row`} key={id}>
            <div
              className={`${cx('Message-sender')} small-4 columns text-right`}
            >
              <strong>
                {sender}
              </strong>
