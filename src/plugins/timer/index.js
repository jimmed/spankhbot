...
export class SettingsPane extends React.Component {
  ...
  render() {
    ...
    const canDelete =
      settings.has('announcements') &&
      settings.get('announcements').count() > 1;
    return (
      <div className={cx('callout')}>
        ...
        <div className="callout warning">
          <p>
            At present, changes will only be applied when connecting to chat.
          </p>
        </div>
        <table>
          <thead>
    ...
    return (
      <tr>
        ...
        <td>
          <input
            type="text"
            value={announcement.get('announcement', '')}
            onChange={this.onChange.bind(this)}
            name="announcement"
          />
        </td>
        <td>
          <button
            type="button"
            className={`button ${announcement.get('enabled')
              ? 'success'
              : 'secondary'}`}
            onClick={this.onToggle.bind(this)}
            name="enabled"
          >
            {announcement.get('enabled') ? 'On' : 'Off'}
          </button>
          {canDelete &&
            <button
              type="button"
...
function enableAnnouncements(settings, responder) {
  const timers = generateTimers(settings).map(({ timer, announcement }) =>
    setInterval(responder, timer, announcement)
  );
...
export function onChatConnect(props) {
  ...
  return enableAnnouncements(settings, send.bind(null, 'bot'));
}
